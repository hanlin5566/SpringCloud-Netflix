...
public class TokenCheckFilter extends ZuulFilter{
    ...
    */
	...
	public Object run() {
        ...
        Object accessToken = request.getParameter("token");  //定义规则：访问url中必须带有token参数
        if(accessToken == null) {
            ctx.setSendZuulResponse(false);
            ctx.setResponseStatusCode(401);
            try {
                ctx.getResponse().getWriter().write(String.format("method:%s,url:%s,error:%s", request.getMethod(),request.getRequestURL(),"token is empty"));
            }catch (Exception e){}
            return null;
        }
        return null;
